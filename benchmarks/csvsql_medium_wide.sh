...
# Data file
data_file="../data/medium_wide.csv"
table_name="medium_wide"
...
# Output CSV header
echo "Query,Total Time,Number of Runs,Average Time" > csvsql_medium_wide_benchmarks.csv
...
for query in "${queries[@]}"; do
    ...
    for i in $(seq 1 $num_runs); do
        if [ $i -eq 1 ]; then
            output_file="csvsql_medium_wide_output.csv"
    ...
    echo "\"$query\",$total_time,$num_runs,$avg_time" >> csvsql_medium_wide_benchmarks.csv
done
