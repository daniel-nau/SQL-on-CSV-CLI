...
# Data file
data_file="../data/large_tall.csv"
table_name="large_tall"
...
# Output CSV header
echo "Query,Total Time,Number of Runs,Average Time" >> csvsql_large_tall_benchmarks.csv
...
for query in "${queries[@]}"; do
    ...
    for i in $(seq 1 $num_runs); do
        if [ $i -eq 1 ]; then
            output_file="csvsql_large_tall_output.csv"
    ...
    echo "\"$query\",$total_time,$num_runs,$avg_time" >> csvsql_large_tall_benchmarks.csv
done
